...
export default function Item({ name, items }: { name: string, items: { name: string, id: number }[] }) {
  return (
          ...
          <>
            ...
            >
              <Popover.Panel className="absolute left-1/2 z-10 mt-3 w-screen max-w-sm transform px-4 sm:px-0 lg:max-w-3xl">
                <div className="overflow-hidden w-52 rounded-lg shadow-lg ring-1 ring-black/5">
                  {items.map((item) => (
                    <div className="relative w-full grid gap-8 bg-white p-7 lg:grid-cols-2" key={item.id}>
                      {item.name}
                    </div>
                  ))}
                </div>
